          ...
          >
            ...
            <q-card-section style="padding: 0px;">
              ...
              <q-chip
                ...
                icon-remove="eva-close"
                class="full-width q-my-xs"
                style="padding: 16px; color: rgb(25, 103, 210); background-color: rgba(26, 115, 232, 0.12);"
...
export default {
  ...
  methods: {
    ...
    editar () {
      ...
      this.parada.fotos = this.urls

      db.collection('paradas').doc(this.parada.id).set(this.parada).then(() => {
        this.fotos.forEach((foto) => {
          const archivo = storage.ref('paradas/' + this.parada.id + '/' + Math.floor(Math.random() * 999999) + ' - ' + foto.name)

          archivo.put(foto).then(() => {
            archivo.getDownloadURL().then((url) => {
              this.urls.push(url)

              db.collection('paradas').doc(this.parada.id).update({ fotos: this.urls })
            })
          })
        })
      })
  ...
  created () {
    eventBus.$on('mostrarModalEditar', (parada) => {
      this.estado = true
      ...
      this.parada = {
        id: String(parada.id),
        nombre: String(parada.nombre),
        referencia: String(parada.referencia),
        ciudad: String(parada.ciudad),
        lugaresCercanos: String(parada.lugaresCercanos),
        gps: new GeoPoint(parada.gps._lat, parada.gps._long),
        fotos: parada.fotos.map(String)
      }

      this.urls = parada.fotos.map(String)

      this.fotosStorage = this.urls.map((url) => {
        return storage.refFromURL(url)
      })

      this.maxFotos = 3 - parada.fotos.length
    })
  }
}
</script>
